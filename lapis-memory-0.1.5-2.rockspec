...
description = {
    ...
    detailed = [[
        ...
        Features:
          ...
          * Encrypted secrets management: AES-256-CBC storage in a local JSON
            file (no DB table). Activate by setting secrets_file path.
            execute_with_secret substitutes {secret} server-side without
            ever returning the raw value.
          ...
          * `memo migrate`: prints the full idempotent schema + all
            migrations to stdout; pipe to psql to bootstrap any new
            project in one line.

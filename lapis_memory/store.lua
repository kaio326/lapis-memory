-- lapis_memory.store
-- Persistence + retrieval. Uses lapis.db for raw SQL.
...
function M.configure(config)
    ...
    if type(ngx) == "table" and ngx.log and ngx.INFO then
        ngx.log(ngx.INFO, "lapis_memory backend: ", _backend)
    end
end
...
function M.search(args)
    ...
    if not reranked then
        ...
        if ngx and ngx.log and ngx.WARN then
            ngx.log(ngx.WARN, "lapis_memory rerank failed: ", tostring(rerr))
        end
        if #rows > limit then
            local trimmed = {}
            for i = 1, limit do trimmed[i] = rows[i] end

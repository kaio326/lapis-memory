-- Phase 16.6 — ConvoMem bench runner.
--
-- End-to-end retrieval-side run of a ConvoMem-style dataset through
-- luamemo. Same shape as the LoCoMo runner but reads from the
-- `convomem` loader (whose schema is documented in
-- eval/datasets/convomem.lua and eval/sidecars/convomem.md). Cross-
-- dataset agreement between this runner and `locomo_run.lua` is the
-- intended fiability check for Phase 16.6: if both agree, the number
-- is trustable; if they diverge, either the schema is wrong or the
-- pipeline saturates differently.
--
-- For each (dialogue_id, qa_index):
--   1. (Once per dialogue) wipe scope `convomem:<embedder>:<dialogue_id>`
--      and write every session as one memory.
--   2. Search the question with `limit = k_max`.
--   3. Hit  = any top-K result's `metadata.session_id` is in
--            `evidence_session_ids`. Rank = position of the first hit.
--
-- Usage:
--   PGHOST=127.0.0.1 PGDATABASE=lm_bruteforce_test \
--     lua5.1 eval/convomem_run.lua \
--       --embedder hash --corpus eval/data/fixtures/convomem_tiny.json \
--       --out eval/results/convomem_hash_smoke.json
--
-- Output: `eval/results/convomem_<embedder>.json` with overall + per-
-- category metrics.
...
if not fh then
    ...
    io.stderr:write("see eval/sidecars/convomem.md for download protocol.\n")
    os.exit(1)
end
fh:close()
...
local rows = convomem.load(args.corpus)
...
print(string.format("embedder=%s  dialogues=%d  qa_pairs=%d  k_max=%d  backend=%s  rerank=%s",
...
for ri, r in ipairs(rows) do
    ...
    if os.getenv("INGEST_MODE") == "perrow" then
        ...
        if batch_err then
            io.stderr:write(("write_many fatal for dlg=%s: %s\n")
                :format(dlg_id, tostring(batch_err)))
        end
    end
    for bi, ar in ipairs(results or {}) do
        if ar.error then
            io.stderr:write(("write failed dlg=%s sess=%s: %s\n")
                :format(dlg_id, tostring(batch[bi].title), tostring(ar.error)))
        end
    end

    for qi, qa in convomem.iter_qa(r) do
        if qa.question and qa.question ~= "" then
            local gold = convomem.qa_gold_sessions(qa)
            local cat  = convomem.category_name(qa.category)
            ...
            if not sresults then
                io.stderr:write(("search failed dlg=%s qa=%d: %s\n")
                    :format(dlg_id, qi, tostring(serr)))
                os.exit(1)
            end
    ...
    if ri % 5 == 0 or ri == #rows then
        io.write(string.format("\r  progress: dlg %d/%d (%d/%d qa)",
...
for cat, b in pairs(by_cat) do report.by_category[cat] = summarise(b) end
...
print(string.format("dialogues=%d  qa_pairs=%d  k_max=%d  elapsed=%ds",
